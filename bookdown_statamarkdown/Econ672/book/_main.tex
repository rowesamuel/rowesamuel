...
\urlstyle{same}
\hypersetup{
  pdftitle={Instrument Variables Revisted},
  pdfauthor={John Doe},
  hidelinks,
  pdfcreator={LaTeX via pandoc}}

\title{Instrument Variables Revisted}
\author{John Doe}
\date{2026-02-07}
...
\section{Naive OLS}\label{naive-ols}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{reg}\NormalTok{ lwage  educ  exper }\BaseNTok{black}\NormalTok{ south married i.smsa}
\end{Highlighting}
\end{Shaded}
...
\section{Estimate Local Average Treatment Effect using IV}\label{estimate-local-average-treatment-effect-using-iv}

Next, we will use our instrument variable estimator to estimate the \(LATE\) for those who comply with the instrument. We will use the \emph{ivregress 2sls} command.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ivregress 2sls lwage (educ=nearc4) exper }\BaseNTok{black}\NormalTok{ south married i.smsa, first }
\end{Highlighting}
\end{Shaded}
...
\section{Test Instrument Relevance}\label{test-instrument-relevance}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{reg}\NormalTok{ educ nearc4 exper }\BaseNTok{black}\NormalTok{ south married i.smsa}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{test}\NormalTok{ nearc4}
\end{Highlighting}
\end{Shaded}
...
\section{Test Monotonicity}\label{test-monotonicity}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{quietly} \KeywordTok{reg}\NormalTok{ educ nearc4 exper }\BaseNTok{black}\NormalTok{ south married smsa}
\KeywordTok{predict}\NormalTok{ dhat}
\end{Highlighting}
\end{Shaded}

Next, we will average outcomes across dhat by using the \emph{egen cut} to get bins of outcomes

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{sum}\NormalTok{ lwage, }\KeywordTok{detail}
\KeywordTok{egen}\NormalTok{ lwage\_bins = }\FunctionTok{cut}\NormalTok{(lwage), }\FunctionTok{at}\NormalTok{(5,5.25,5.5,5.75,6,6.25,6.5,6.75,7,7.25,7.5)}
\KeywordTok{sort}\NormalTok{ lwage\_bins}
\KeywordTok{by}\NormalTok{ lwage\_bins: }\KeywordTok{egen}\NormalTok{ mean\_z=}\KeywordTok{mean}\NormalTok{(nearc4)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{*Show Monotonicity Graph }
\KeywordTok{sort}\NormalTok{ mean\_z}
\KeywordTok{twoway} \KeywordTok{line}\NormalTok{ lwage\_bins mean\_z}
\end{Highlighting}
\end{Shaded}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{monotonicity1.png}}
\caption{Monotonicity Assumption Test}
\end{figure}

It is possible that monotonicity assumption is violated.

\chapter{Jackknife Instrument Variable Estimator}\label{jackknife-instrument-variable-estimator}
...
\begin{Highlighting}[]
\NormalTok{cd }\StringTok{"/Users/Sam/Desktop/Econ 672/Data"}
\KeywordTok{use}\NormalTok{ judge\_fe, }\KeywordTok{clear}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{*Set up }\KeywordTok{global}\NormalTok{ macros }
\KeywordTok{global}\NormalTok{ judge\_pre judge\_pre\_1 judge\_pre\_2 judge\_pre\_3 judge\_pre\_4 judge\_pre\_5 judge\_pre\_6 judge\_pre\_7 judge\_pre\_8}
\KeywordTok{global}\NormalTok{ demo }\BaseNTok{black}\NormalTok{ age male }\BaseNTok{white} 
\KeywordTok{global} \KeywordTok{off}\NormalTok{      fel mis }\KeywordTok{sum}\NormalTok{ F1 F2 F3 }\FunctionTok{F}\NormalTok{ M1 M2 M3 M }
\KeywordTok{global}\NormalTok{ prior priorCases priorWI5 prior\_felChar  prior\_guilt onePrior threePriors}
\KeywordTok{global}\NormalTok{ control2     }\FunctionTok{day}\NormalTok{ day2 day3  bailDate t1 t2 t3 t4 t5 t6}
\end{Highlighting}
\end{Shaded}

\section{Naive OLS}\label{naive-ols-1}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{eststo minimal: }\KeywordTok{quietly} \KeywordTok{reg}\NormalTok{ guilt jail3 }\OtherTok{$control2}\NormalTok{, }\KeywordTok{robust}
\NormalTok{eststo maximum: }\KeywordTok{quietly} \KeywordTok{reg}\NormalTok{ guilt jail3 possess robbery DUI1st drugSell aggAss }\OtherTok{$demo} \OtherTok{$prior} \OtherTok{$off}  \OtherTok{$control2}\NormalTok{ , }\KeywordTok{robust}

\NormalTok{esttab minimal maximum, mtitle(}\StringTok{"Minimal"} \StringTok{"Max Controls"}\NormalTok{) }\KeywordTok{keep}\NormalTok{(jail3)}
\end{Highlighting}
\end{Shaded}
...
\section{Instrument Variables}\label{instrument-variables}
...
\NormalTok{eststo rf1: }\KeywordTok{quietly} \KeywordTok{reg}\NormalTok{ jail3 }\OtherTok{$judge\_pre} \OtherTok{$control2}\NormalTok{, }\KeywordTok{robust}
\NormalTok{eststo rf2: }\KeywordTok{quietly} \KeywordTok{reg}\NormalTok{ jail3 possess robbery DUI1st drugSell aggAss }\OtherTok{$demo} \OtherTok{$prior} \OtherTok{$off}  \OtherTok{$control2} \OtherTok{$judge\_pre}\NormalTok{, }\KeywordTok{robust}
\NormalTok{esttab rf1 rf2, mtitle(}\StringTok{"Min Controls"} \StringTok{"Max Controls"}\NormalTok{) }\KeywordTok{keep}\NormalTok{(judge\_pre\_1 judge\_pre\_2 judge\_pre\_3 judge\_pre\_4 judge\_pre\_5 judge\_pre\_6 judge\_pre\_7 judge\_pre\_8)}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{reg}\NormalTok{ jail3 }\OtherTok{$judge\_pre} \OtherTok{$control2}\NormalTok{, }\KeywordTok{robust}
\NormalTok{\}}
\KeywordTok{test} \OtherTok{$judge\_pre}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\KeywordTok{quietly}\NormalTok{ \{}
\KeywordTok{reg}\NormalTok{ jail3 }\OtherTok{$judge\_pre}\NormalTok{ possess robbery DUI1st drugSell aggAss }\OtherTok{$demo} \OtherTok{$prior} \OtherTok{$off} \OtherTok{$control2}\NormalTok{ , }\KeywordTok{robust} 
\NormalTok{\}}
\KeywordTok{test} \OtherTok{$judge\_pre}
\end{Highlighting}
\end{Shaded}
...
Now let's estimate our two models with an IV estimator with the \emph{ivregress 2sls} command.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{est} \KeywordTok{clear}
\NormalTok{eststo iv1: }\KeywordTok{quietly}\NormalTok{ ivregress 2sls guilt (jail3= }\OtherTok{$judge\_pre}\NormalTok{) }\OtherTok{$control2}\NormalTok{, }\KeywordTok{robust}\NormalTok{ first}
\NormalTok{eststo iv2: }\KeywordTok{quietly}\NormalTok{ ivregress 2sls guilt (jail3= }\OtherTok{$judge\_pre}\NormalTok{) possess robbery DUI1st drugSell aggAss }\OtherTok{$demo} \OtherTok{$prior} \OtherTok{$off} \OtherTok{$control2}\NormalTok{ , }\KeywordTok{robust}\NormalTok{ first}
\NormalTok{esttab iv1 iv2, mtitle(}\StringTok{"Min IV Model"} \StringTok{"Max IV Model"}\NormalTok{) }\KeywordTok{keep}\NormalTok{(jail3)}
\end{Highlighting}
\end{Shaded}
...
\section{Jackknife Instrument Variable Estimator}\label{jackknife-instrument-variable-estimator-1}

Now we will implement our Jackknife IV estimator and estimate miminal-control and maximum-control models. We use the \emph{jive} command after doing our install of package st0108.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{jive guilt (jail3= }\OtherTok{$judge\_pre}\NormalTok{) }\OtherTok{$control2}\NormalTok{, }\KeywordTok{robust}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{jive guilt (jail3= }\OtherTok{$judge\_pre}\NormalTok{) possess robbery DUI1st drugSell aggAss }\OtherTok{$demo} \OtherTok{$prior} \OtherTok{$off} \OtherTok{$control2}\NormalTok{ , }\KeywordTok{robust}
\end{Highlighting}
\end{Shaded}
...
Angrist, J.D. and Krueger, A.B. (2001). Instrument variables and the search for identification: From supply and demand to natural experiments. \emph{Journal of Economic Perspectives}. 15(4) p.69-85.
...
Stevenson, M.T. (2018). Distortion of justice: How the inability to pay bail affects case outcomes. \emph{Journal of Law, Economics, and Organization}. 34(4) p.511-542.
