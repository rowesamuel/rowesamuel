...
\urlstyle{same}
\hypersetup{
  pdftitle={Two-Way Fixed Effect with Differential Timing and Goodman-Bacon Decomposition},
  pdfauthor={Samuel Rowe adapted from Cunningham (2021)},
  hidelinks,
  pdfcreator={LaTeX via pandoc}}

\title{Two-Way Fixed Effect with Differential Timing and Goodman-Bacon Decomposition}
...
\chapter{Two-way Fixed Effects with Differential Timing}\label{two-way-fixed-effects-with-differential-timing}

\section{Replicate Cheng and Hoesktra (2013)}\label{replicate-cheng-and-hoesktra-2013}
...
We have a few option for implementing Goodman-Bacon Decomposition. The first one is the \emph{bacondecomp} package and the other is a post-estimation command with \emph{xtdidregress}.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{ssc}\NormalTok{ install bacondecomp}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{use}\NormalTok{ castle}

\KeywordTok{sum}\NormalTok{ homicide, }\KeywordTok{detail}
\end{Highlighting}
\end{Shaded}
...
99%     12.92404       14.52561       Kurtosis       3.558378
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{histogram homicide}
\end{Highlighting}
\end{Shaded}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Homicides per 100K}]{CH1.png}}
\caption{Homicides per 100K}
\end{figure}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{histogram l\_homicide, xtitle("Natural Log of Homicides per 100K")}
\end{Highlighting}
\end{Shaded}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Natural Log of Homicides per 100K}]{CH2.png}}
\caption{Natural Log of Homicides per 100K}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{label} \KeywordTok{variable} \KeywordTok{post} \StringTok{"Year of treatment"}
\KeywordTok{tab} \FunctionTok{year} \KeywordTok{post}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{global}\NormalTok{ crime1 jhcitizen\_c jhpolice\_c murder homicide  robbery assault burglary larceny motor robbery\_gun\_r }
\KeywordTok{global}\NormalTok{ demo blackm\_15\_24 whitem\_15\_24 blackm\_25\_44 whitem\_25\_44 }\CommentTok{//demographics}
\KeywordTok{global}\NormalTok{ lintrend trend\_1{-}trend\_51 }\CommentTok{//state linear trend}
\KeywordTok{global}\NormalTok{ region r20001{-}r20104  }\CommentTok{//region{-}quarter fixed effects}
\KeywordTok{global}\NormalTok{ exocrime l\_larceny l\_motor }\CommentTok{// exogenous crime rates}
\KeywordTok{global}\NormalTok{ spending l\_exp\_subsidy l\_exp\_pubwelfare }
\KeywordTok{global}\NormalTok{ xvar l\_police unemployrt poverty l\_income l\_prisoner l\_lagprisoner }\OtherTok{$demo} \OtherTok{$spending}
\end{Highlighting}
\end{Shaded}

\section{TWFE with Differential Timing}\label{twfe-with-differential-timing}

We will use our \emph{xtreg} command to estimate the TWFE estimator.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{xtset sid }\FunctionTok{year}
\KeywordTok{xtreg}\NormalTok{ l\_homicide i.}\FunctionTok{year} \OtherTok{$region} \OtherTok{$xvar} \OtherTok{$lintrend} \KeywordTok{post}\NormalTok{ [}\KeywordTok{aweight}\NormalTok{=popwt], }\KeywordTok{fe} \KeywordTok{vce}\NormalTok{(}\KeywordTok{cluster}\NormalTok{ sid)}
\end{Highlighting}
\end{Shaded}
...
We can also use \emph{xtdidregress}, which gives a concise estimate of the \(ATET\) and allows for us to test for pre-treatment trends.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{xtdidregress (l\_homicide i.}\FunctionTok{year} \OtherTok{$region} \OtherTok{$xvar} \OtherTok{$lintrend}\NormalTok{) (}\KeywordTok{post}\NormalTok{) [}\KeywordTok{aweight}\NormalTok{=popwt], }\FunctionTok{group}\NormalTok{(sid) time(}\FunctionTok{year}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{xtevent l\_homicide }\OtherTok{$region}\NormalTok{ [}\KeywordTok{aweight}\NormalTok{=popwt], pol(}\KeywordTok{post}\NormalTok{) }\KeywordTok{cluster}\NormalTok{(sid) timevar(}\FunctionTok{year}\NormalTok{) }\KeywordTok{window}\NormalTok{(}\FunctionTok{max}\NormalTok{) }\KeywordTok{impute}\NormalTok{(stag) reghdfe }
\end{Highlighting}
\end{Shaded}
...
The calculated window by window(max) is (-9,3), plus the endpoints -10 and 4.
(converged in 3 iterations)
note: r20002 omitted because of collinearity
note: r20013 omitted because of collinearity
note: r20024 omitted because of collinearity
note: r20032 omitted because of collinearity
note: r20034 omitted because of collinearity
note: r20043 omitted because of collinearity
note: r20054 omitted because of collinearity
note: r20062 omitted because of collinearity
note: r20072 omitted because of collinearity
note: r20083 omitted because of collinearity
note: r20084 omitted because of collinearity
note: r20092 omitted because of collinearity
note: r20101 omitted because of collinearity
note: r20102 omitted because of collinearity

HDFE Linear regression                            Number of obs   =        550
Absorbing 2 HDFE groups                           F(  44,     49) =     295.06
Statistics robust to heteroskedasticity           Prob > F        =     0.0000
                                                  R-squared       =     0.9444
                                                  Adj R-squared   =     0.9315
                                                  Within R-sq.    =     0.1807
Number of clusters (sid)     =         50         Root MSE        =     0.1083
...
------------------------------------------------------------------------------
  ...
  l_homicide | Coefficient  std. err.      t    P>|t|     [95% conf. interval]
-------------+----------------------------------------------------------------
   _k_eq_m10 |  -.2670587   .0464639    -5.75   0.000    -.3604314    -.173686
    _k_eq_m9 |  -.3136291   .0810769    -3.87   0.000    -.4765593    -.150699
    _k_eq_m8 |  -.1480105   .0852785    -1.74   0.089    -.3193841    .0233631
    _k_eq_m7 |  -.0011922   .0574174    -0.02   0.984    -.1165768    .1141923
    _k_eq_m6 |  -.0033678   .0473356    -0.07   0.944    -.0984923    .0917567
    _k_eq_m5 |  -.0187262   .0486225    -0.39   0.702    -.1164367    .0789844
    _k_eq_m4 |   .0085331   .0356398     0.24   0.812    -.0630878     .080154
    _k_eq_m3 |   .0049404   .0339288     0.15   0.885    -.0632421    .0731229
    _k_eq_m2 |  -.0322577    .034501    -0.93   0.354      -.10159    .0370745
    _k_eq_p0 |   .0758257   .0281218     2.70   0.010     .0193127    .1323386
    _k_eq_p1 |   .0648564   .0475352     1.36   0.179    -.0306691    .1603819
    _k_eq_p2 |   .0905699   .0586977     1.54   0.129    -.0273875    .2085274
    _k_eq_p3 |   .0918709   .0565459     1.62   0.111    -.0217624    .2055042
    _k_eq_p4 |   .1844643    .056076     3.29   0.002     .0717754    .2971532
      r20001 |  -.3176814   .1055113    -3.01   0.004    -.5297144   -.1056485
      r20002 |          0  (omitted)
      r20003 |  -.1014046   .0678023    -1.50   0.141    -.2376584    .0348492
      r20004 |  -.1735281   .0529523    -3.28   0.002    -.2799397   -.0671165
      r20011 |   -.097969   .0996036    -0.98   0.330    -.2981299     .102192
      r20012 |    .136426   .0652085     2.09   0.042     .0053847    .2674674
      r20013 |          0  (omitted)
      r20014 |   .0644022   .0543544     1.18   0.242    -.0448271    .1736314
      r20021 |  -.2108039   .0728759    -2.89   0.006    -.3572534   -.0643543
      r20022 |   .0004829    .048146     0.01   0.992    -.0962701     .097236
      r20023 |  -.0783909    .046152    -1.70   0.096    -.1711368     .014355
      r20024 |          0  (omitted)
      r20031 |  -.1710896    .079739    -2.15   0.037    -.3313312    -.010848
      r20032 |          0  (omitted)
      r20033 |  -.0556467   .0549892    -1.01   0.317    -.1661516    .0548581
      r20034 |          0  (omitted)
      r20041 |    -.08143   .0741366    -1.10   0.277     -.230413     .067553
      r20042 |   .0413541   .0596014     0.69   0.491    -.0784194    .1611277
      r20043 |          0  (omitted)
      r20044 |   .1008408   .0384528     2.62   0.012      .023567    .1781145
      r20051 |  -.1464866   .0682065    -2.15   0.037    -.2835527   -.0094205
      r20052 |   -.002504   .0601266    -0.04   0.967    -.1233329    .1183249
      r20053 |  -.0910554   .0515309    -1.77   0.083    -.1946106    .0124998
      r20054 |          0  (omitted)
      r20061 |  -.1217555   .0545111    -2.23   0.030    -.2312997   -.0122112
      r20062 |          0  (omitted)
      r20063 |  -.0808163   .0693046    -1.17   0.249    -.2200891    .0584566
      r20064 |  -.0086811   .0605718    -0.14   0.887    -.1304046    .1130425
      r20071 |  -.1647261   .0589828    -2.79   0.007    -.2832565   -.0461957
      r20072 |          0  (omitted)
      r20073 |  -.0110104   .0615684    -0.18   0.859    -.1347367    .1127159
      r20074 |  -.0173833   .0625427    -0.28   0.782    -.1430676    .1083011
      r20081 |  -.0441083   .0705833    -0.62   0.535    -.1859507    .0977342
      r20082 |   .0834497   .0698646     1.19   0.238    -.0569485    .2238478
      r20083 |          0  (omitted)
      r20084 |          0  (omitted)
      r20091 |  -.1250185   .0483505    -2.59   0.013    -.2221826   -.0278545
      r20092 |          0  (omitted)
      r20093 |   -.059229   .0684898    -0.86   0.391    -.1968645    .0784065
      r20094 |  -.0440906   .0717044    -0.61   0.541     -.188186    .1000047
      r20101 |          0  (omitted)
      r20102 |          0  (omitted)
      r20103 |  -.1376362   .0624248    -2.20   0.032    -.2630836   -.0121889
      r20104 |  -.1100676     .06304    -1.75   0.087    -.2367512     .016616
------------------------------------------------------------------------------
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{xteventplot, }\BaseNTok{xtitle}\NormalTok{(}\StringTok{"Years from policy"}\NormalTok{) }\BaseNTok{ytitle}\NormalTok{(}\StringTok{"LN of Homicide Rate per 100K"}\NormalTok{) }\BaseNTok{title}\NormalTok{(}\StringTok{"Event Study of Cheng and Hoesktra (2013)"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Event Study of Cheng and Hoesktra (2013)}]{CH3.png}}
\caption{Event Study of Cheng and Hoesktra (2013)}
\end{figure}
...
\chapter{Goodman-Bacon Decomposition}\label{goodman-bacon-decomposition}

We will again use the data from Cheng and Hoesktra (2013) to assess the Bacon-Goodman Decomposition of the different \(ATETs\) when there is differential timing. We have a few options to calculate the individual \(ATETs\) and different variance weights. We will focus on \emph{estat bdecomp}, \emph{bacondecomp} and \emph{ddtimg}. Please note that \emph{ddtiming} has been depreciated.

\section{\texorpdfstring{\emph{estat bdecomp}}{estat bdecomp}}\label{estat-bdecomp}

Our first method to implement the Bacon-Goodman Decomposition is \emph{estat bdecomp}. We run this command after we use \emph{xtdidregress} or \emph{didregress}. Please note that we use the option \emph{graph} to display the individual \(ATET\) and the corresponding weights.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{xtdidregress (l\_homicide) (}\KeywordTok{post}\NormalTok{), }\FunctionTok{group}\NormalTok{(sid) time(}\FunctionTok{year}\NormalTok{)}
\KeywordTok{estat}\NormalTok{ bdecomp, }\KeywordTok{graph}
\end{Highlighting}
\end{Shaded}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Bacon-Goodman Decomposition}]{CH4.png}}
...
\section{\texorpdfstring{\emph{bacondecomp}}{bacondecomp}}\label{bacondecomp}

Our second method to implement the Bacon-Goodman Decomposition is \emph{bacondecomp}
...
We will first run a TWFE regression with \emph{areg} and then run Bacondecomp after regression, but it generates stubs. Notice that the \emph{areg} will have similar outcome as our \emph{xtdidregress}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{areg}\NormalTok{ l\_homicide }\KeywordTok{post}\NormalTok{ i.}\FunctionTok{year}\NormalTok{, absorb(sid) }\KeywordTok{robust}
\NormalTok{bacondecomp l\_homicide }\KeywordTok{post}\NormalTok{, stub(Bacon\_) }\KeywordTok{robust}\NormalTok{ ddetail}
\KeywordTok{drop}\NormalTok{ Bacon\_*}
\end{Highlighting}
\end{Shaded}
...
Notice that the TWFE estimator is the same between \emph{areg} and \emph{xtdidregress}. One thing about this command is that we lack details of the individual groups. We only see Early\_v\_Late and Late\_v\_Early. I personally prefer the \emph{estat bdecomp} command.

\section{\texorpdfstring{\emph{ddtiming}}{ddtiming}}\label{ddtiming}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{net install ddtiming, from(https:}\CommentTok{//raw.githubusercontent.com/tgoldring/ddtiming/master)}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ddtiming l\_homicide }\KeywordTok{post}\NormalTok{, i(sid) t(}\FunctionTok{year}\NormalTok{) }\KeywordTok{yline}\NormalTok{(0) }
\end{Highlighting}
\end{Shaded}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Bacon-Goodman Decomposition with ddtiming}]{CH5.png}}
\caption{Bacon-Goodman Decomposition with \emph{ddtiming}}
\end{figure}

We get similar results, but the issue is that \emph{ddtiming} has been depreciated.
