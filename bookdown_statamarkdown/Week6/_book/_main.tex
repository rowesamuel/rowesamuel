...
\urlstyle{same}
\hypersetup{
  pdftitle={Week 6: Difference-in-Differences and EGEN Commands},
  pdfauthor={Samuel Rowe adapted from Wooldridge (2011) and Mitchell (2020)},
  hidelinks,
  pdfcreator={LaTeX via pandoc}}

\title{Week 6: Difference-in-Differences and EGEN Commands}
...
We will be covering difference-in-differences and two-way fixed effects estimators. We will also go over the \emph{egen} commands for data management.
...
\chapter{Difference-in-Differences and Two-Way Fixed Effects}\label{difference-in-differences-and-two-way-fixed-effects}
...
\section{Diff-in-Diff}\label{diff-in-diff}

We will cover the implementation of the difference-in-difference research design. We only need the \emph{reg} command to implement the design, but we will utilize the interaction operator. You only need pooled data for a difference-in-difference research design.

\subsection{Effect of a garbage incinerator's location on housing prices}\label{effect-of-a-garbage-incinerators-location-on-housing-prices}

\textbf{Lesson:} The 2-by-2 Difference-in-Difference estimator is simple to implement
...
\KeywordTok{use} \StringTok{"kielmc.dta"}\NormalTok{, }\KeywordTok{clear}
\KeywordTok{reg}\NormalTok{ rprice nearinc }\KeywordTok{if}\NormalTok{ y81==1}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{reg}\NormalTok{ rprice nearinc }\KeywordTok{if}\NormalTok{ y81==0}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{reg}\NormalTok{ rprice i.nearinc\#\#i.y81}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{eststo m1: }\KeywordTok{quietly} \KeywordTok{reg}\NormalTok{ rprice i.nearinc\#\#i.y81}
\NormalTok{eststo m2: }\KeywordTok{quietly} \KeywordTok{reg}\NormalTok{ rprice i.nearinc\#\#i.y81 age agesq}
\NormalTok{eststo m3: }\KeywordTok{quietly} \KeywordTok{reg}\NormalTok{ rprice i.nearinc\#\#i.y81 age agesq intst land area rooms baths}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{esttab m1 m2 m3, }\KeywordTok{keep}\NormalTok{(1.y81 1.nearinc 1.nearinc\#1.y81 age agesq intst land area rooms baths)}
\end{Highlighting}
\end{Shaded}
...
Let's use elasticities by usig a log-linear model. We'll use the \emph{estimates store} command for plotting the coefficients.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{est} \KeywordTok{clear}
\NormalTok{eststo m1: }\KeywordTok{quietly} \KeywordTok{reg}\NormalTok{ lprice i.nearinc\#\#i.y81}
\KeywordTok{estimates} \KeywordTok{store}\NormalTok{ mod1}
\NormalTok{eststo m2: }\KeywordTok{quietly} \KeywordTok{reg}\NormalTok{ lprice i.nearinc\#\#i.y81 age agesq}
\KeywordTok{estimates} \KeywordTok{store}\NormalTok{ mod2}
\NormalTok{eststo m3: }\KeywordTok{quietly} \KeywordTok{reg}\NormalTok{ lprice i.nearinc\#\#i.y81 age agesq intst land area rooms baths}
\KeywordTok{estimates} \KeywordTok{store}\NormalTok{ mod3}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{coefplot (mod1, }\KeywordTok{label}\NormalTok{(Model 1)) (mod2, }\KeywordTok{label}\NormalTok{(Model2)) (mod3, }\KeywordTok{label}\NormalTok{(Model 3)), }\KeywordTok{keep}\NormalTok{(1.nearinc\#1.y81) }\BaseNTok{xline}\NormalTok{(0) }\BaseNTok{title}\NormalTok{(}\StringTok{"Diff{-}in{-}Diff Results"}\NormalTok{)}
\KeywordTok{graph} \KeywordTok{export} \StringTok{"/Users/Sam/Desktop/Econ 645/Stata/week5\_housing.png"}\NormalTok{, }\KeywordTok{replace}
\end{Highlighting}
\end{Shaded}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{week5_housing.png}}
\caption{Coefficients}
\end{figure}

More on coefplot:
\url{https://repec.sowi.unibe.ch/stata/coefplot/getting-started.html}
...
\subsection{Effect of Worker Compensation Laws on Weeks out of Work}\label{effect-of-worker-compensation-laws-on-weeks-out-of-work}

\textbf{Lesson:} We need a comparison group for the parallel trends assumption.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cd }\StringTok{"/Users/Sam/Desktop/Econ 645/Data/Wooldridge"}
\KeywordTok{use} \StringTok{"injury.dta"}\NormalTok{, }\KeywordTok{clear}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{tab}\NormalTok{ highearn afchnge}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{reg}\NormalTok{ ldurat i.afchnge\#\#i.highearn }\KeywordTok{if}\NormalTok{ ky==1}
\KeywordTok{reg}\NormalTok{ ldurat i.afchnge\#\#i.highearn i.male i.married i.indust i.injtype }\KeywordTok{if}\NormalTok{ ky==1}
\end{Highlighting}
\end{Shaded}
...
\subsubsection*{Not in Wooldridge:}\label{not-in-wooldridge}
\addcontentsline{toc}{subsubsection}{Not in Wooldridge:}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{reg}\NormalTok{ ldurat i.afchnge\#\#i.highearn\#\#i.ky }
\KeywordTok{reg}\NormalTok{ ldurat i.afchnge\#\#i.highearn\#\#i.ky i.male i.married i.indust i.injtype}
\end{Highlighting}
\end{Shaded}
...
\textbf{Questions:} Are worker compensation trends similar between low earners and high earners? Why or why not? Would Michigan make a good comparison group? Can we test this?

\section{Two-Way Fixed Effects}\label{two-way-fixed-effects}

When we utilize a two-way fixed effects research design, we will need a panel and use \emph{xtset} command to set the panel. We can use the \emph{d.} operator for a first-difference regression or \emph{xtreg} command for a fixed effects regression

\subsection{Effect of Grants on scrap rates}\label{effect-of-grants-on-scrap-rates}

\textbf{Lesson:} We can look at individual firm fixed effects and time fixed effects or a two-way fixed effects method. This is very similar to our Diff-in-Diff method in certain ways, but we have staggered adoption of the program.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cd }\StringTok{"/Users/Sam/Desktop/Econ 645/Data/Wooldridge"}
\KeywordTok{use} \StringTok{"jtrain.dta"}\NormalTok{, }\KeywordTok{clear}
\end{Highlighting}
\end{Shaded}

Michigan implemented a job training grant program to reduce scrap rates. What is the effect of job training on reducing the scrap rate for \$ firm\_i \$ during time period \$ t \$ in terms of number of items scraped per 100 due to
defects?
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{sort}\NormalTok{ fcode }\FunctionTok{year}
\NormalTok{xtset fcode }\FunctionTok{year}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{reg} \KeywordTok{d}\NormalTok{.scrap }\KeywordTok{d}\NormalTok{.grant }\KeywordTok{if} \FunctionTok{year}\NormalTok{ \textless{} 1989}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{xtreg}\NormalTok{ scrap i.grant i.d88 }\KeywordTok{if} \FunctionTok{year}\NormalTok{ \textless{} 1989, }\KeywordTok{fe}
\end{Highlighting}
\end{Shaded}
...
\subsection{Effect of Drunk Driving Laws on Traffic Fatalities}\label{effect-of-drunk-driving-laws-on-traffic-fatalities}

\textbf{Lesson:} We can try to evaluate the effect of drunk driving laws by controlling for unobserved time-invariant effects at the state level by looking at states that changed their laws.
...
\begin{Highlighting}[]
\NormalTok{cd }\StringTok{"/Users/Sam/Desktop/Econ 645/Data/Wooldridge"}
\KeywordTok{use} \StringTok{"traffic1.dta"}\NormalTok{, }\KeywordTok{clear}
\KeywordTok{tab}\NormalTok{ copen}
\KeywordTok{tab}\NormalTok{ open85 open90}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{reg}\NormalTok{ cdthrte copen cadmn}
\end{Highlighting}
\end{Shaded}
...
\def\labelenumi{\arabic{enumi}.}
\item
  What is a potential problem with using a binary variable (nearinc) from a continuous variable (dist)?
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{use} \StringTok{"traffic1\_reshaped.dta"}\NormalTok{, }\KeywordTok{clear}
\end{Highlighting}
\end{Shaded}
...
\chapter{EGEN Command}\label{egen-command}

\section{EGEN Computations across variables}\label{egen-computations-across-variables}
